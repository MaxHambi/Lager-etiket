...
var require_ITF14 = __commonJS({
  "node_modules/jsbarcode/bin/barcodes/ITF/ITF14.js"(exports) {
    ...
    var checksum = function checksum2(data) {
      var res = data.substr(0, 13).split("").map(function(num2) {
...
function $(id) {
  ...
  if (!el) throw new Error(`Element mit ID "${id}" nicht gefunden \u2014 index.html und src/ui sind nicht synchron.`);
  return el;
}
...
var Logger = class {
  ...
  save() {
    const text = Array.from(this.el.children).map((el) => el.textContent).join("\n");
    downloadBlob(new Blob([text], { type: "text/plain;charset=utf-8" }), "generator-protokoll.txt");
  }
};

// src/ui/template-picker.ts
var TemplatePicker = class {
  constructor(log) {
    ...
    const tplFileInput = $("tplFile");
    const tplPreviewImg = $("tplPreviewImg");
   ...
   */
  loadFile(file, previewImg, previewWrap, meta) {
    if (!file || !/image\/png|image\//.test(file.type)) {
      this.log.err("Bitte eine PNG-Bilddatei als Vorlage ausw\xE4hlen.");
      return;
    }
    ...
    img.onload = () => {
      ...
      meta.textContent = file.name + " \u2014 " + img.naturalWidth + "\xD7" + img.naturalHeight + " px";
      this.log.ok("Vorlage geladen: " + file.name + " (" + img.naturalWidth + "\xD7" + img.naturalHeight + " px)");
...
function parseEntries(raw) {
  ...
  lines.forEach((rawLine, idx) => {
    ...
    if (seen.has(line)) {
      throw new Error(
        'Doppelter Eintrag "' + line + '" in Zeile ' + (idx + 1) + " (zuerst in Zeile " + seen.get(line) + "). Abgebrochen, bevor doppelte Schilder entstehen."
      );
    }
    seen.set(line, idx + 1);
    entries.push(line);
...
var EntriesUI = class {
  constructor(log) {
    ...
    entriesFile.addEventListener("change", (e) => {
      ...
      reader.onload = () => {
        entriesText.value = reader.result;
...
function readConfig() {
  const areaAuto = $("areaAuto");
  const areaWidth = areaAuto.checked ? null : num($("cfgWidth").value, 0);
  const areaHeight = areaAuto.checked ? null : num($("cfgHeight2").value, 0);
  return {
    barcode: {
      height: num($("cfgHeight").value, 180),
      barWidth: num($("cfgBarWidth").value, 4),
      margin: num($("cfgMargin").value, 20),
      fontSize: num($("cfgFontSize").value, 54),
      fontFamily: $("cfgFontFamily").value || "Arial, Helvetica, sans-serif",
      color: $("cfgColor").value || "#000000",
      background: "transparent",
      textMargin: num($("cfgTextMargin").value, 6)
    },
    placement: {
      area: {
        left: num($("cfgLeft").value, 0),
        top: num($("cfgTop").value, 0),
        width: areaWidth,
        height: areaHeight
      },
      maxWidthPercent: num($("cfgMaxW").value, 90),
      maxHeightPercent: num($("cfgMaxH").value, 90),
      offsetX: num($("cfgOffX").value, 0),
      offsetY: num($("cfgOffY").value, 0)
    },
    output: {
      prefix: $("cfgPrefix").value || "lagerplatz_",
      dpi: num($("cfgDpi").value, 300),
      overwrite: false
    }
  };
}
function applyConfig(cfg) {
  ...
  const a = p.area;
  const o = cfg.output;
  $("cfgHeight").value = String(b.height ?? 180);
  $("cfgBarWidth").value = String(b.barWidth ?? 4);
  $("cfgMargin").value = String(b.margin ?? 20);
  $("cfgFontSize").value = String(b.fontSize ?? 54);
  $("cfgFontFamily").value = b.fontFamily ?? "Arial, Helvetica, sans-serif";
  $("cfgColor").value = b.color ?? "#000000";
  $("cfgTextMargin").value = String(b.textMargin ?? 6);
  $("cfgLeft").value = String(a.left ?? 0);
  $("cfgTop").value = String(a.top ?? 0);
  const isAuto = a.width == null || a.height == null;
  $("areaAuto").checked = isAuto;
  $("cfgWidth").value = a.width == null ? "" : String(a.width);
  $("cfgHeight2").value = a.height == null ? "" : String(a.height);
  $("cfgMaxW").value = String(p.maxWidthPercent ?? 90);
  $("cfgMaxH").value = String(p.maxHeightPercent ?? 90);
  $("cfgOffX").value = String(p.offsetX ?? 0);
  $("cfgOffY").value = String(p.offsetY ?? 0);
  $("cfgPrefix").value = o.prefix ?? "lagerplatz_";
  $("cfgDpi").value = String(o.dpi ?? 300);
  toggleAreaFields();
}
function toggleAreaFields() {
  const auto = $("areaAuto").checked;
  const widthInput = $("cfgWidth");
  const heightInput = $("cfgHeight2");
...
var ConfigUI = class {
  constructor(log) {
    ...
    configFile.addEventListener("change", (e) => {
      ...
      reader.onload = () => {
        try {
          ...
          this.log.err("config.json konnte nicht gelesen werden: " + err.message);
        }
      };
      reader.readAsText(file, "utf-8");
    });
    $("btnResetConfig").addEventListener("click", () => {
      ...
      this.log.info("Konfiguration auf Standardwerte zur\xFCckgesetzt.");
    });
...
function renderBarcodeCanvas(text, cfg) {
  const canvas = document.createElement("canvas");
  (0, import_jsbarcode.default)(canvas, text, {
    format: "CODE128",
    displayValue: true,
    text,
    width: cfg.barWidth,
...
// src/core/compose.ts
function composeLabel(text, templateImg, cfg, warn) {
  ...
  if (left + areaWidth > tplW || top + areaHeight > tplH) {
    const msg = "Zielbereich (" + left + "," + top + "," + areaWidth + "x" + areaHeight + ") reicht \xFCber die Vorlagengr\xF6\xDFe (" + tplW + "x" + tplH + ") hinaus.";
    if (warn) warn(msg);
  }
  ...
  show() {
    ...
    try {
      ...
      if ($("showArea").checked) {
        ...
        ctx.save();
        const accent = getComputedStyle(document.documentElement).getPropertyValue("--preview-overlay").trim() || "rgba(203,166,247,0.9)";
      ...
      this.log.ok(
        'Vorschau erzeugt f\xFCr "' + entry + '" \u2014 Barcode ' + result.finalWidth + "\xD7" + result.finalHeight + " px @ (" + result.posLeft + "," + result.posTop + ")"
      );
    } catch (err) {
      this.log.err("Vorschau fehlgeschlagen: " + err.message);
    }
  }
};
...
// src/core/png.ts
var IHDR_END = 33;
async function injectPhysDpi(pngBlob, dpi) {
  const buf = new Uint8Array(await pngBlob.arrayBuffer());
  if (buf.length < IHDR_END || buf[12] !== 73 || buf[13] !== 72 || buf[14] !== 68 || buf[15] !== 82) {
    return pngBlob;
  }
...
// src/ui/generator.ts
var GeneratorUI = class {
  constructor(log, templates, entries) {
    __publicField(this, "log", log);
    __publicField(this, "templates", templates);
    __publicField(this, "entries", entries);
  ...
  async generateAll() {
    ...
    let created = 0;
    this.log.info(entries.length + " Eintr\xE4ge gefunden. Erzeuge Schilder \u2026");
    for (let i = 0; i < entries.length; i++) {
      ...
      try {
        ...
        this.log.err('Fehler bei "' + entry + '": ' + err.message);
      }
   ...
   */
  addThumb(grid, fileName, url, entry) {
    const div = document.createElement("div");
    div.className = "thumb";
    const safeEntry = this.escapeHtml(entry);
    const safeName = this.escapeHtml(fileName);
    div.innerHTML = '<img src="' + url + '" alt="' + safeEntry + '"><div class="cap"><span title="' + safeName + '">' + safeName + '</span><a href="' + url + '" download="' + safeName + '">\u2193</a></div>';
...
function bootstrap() {
  ...
  preview.updateEnabled();
  generator.updateEnabled();
  log.info("Werkzeug bereit. Vorlage laden und Lagerpl\xE4tze eintragen, um zu starten.");
  log.info("JsBarcode v3.12.3 (CODE128) via npm \u2014 l\xE4uft vollst\xE4ndig offline.");
}
