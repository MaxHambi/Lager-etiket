...
export function composeLabel(
  ...
  if (left + areaWidth > tplW || top + areaHeight > tplH) {
    const msg =
      "Zielbereich (" + left + "," + top + "," + areaWidth + "x" + areaHeight +
      ") reicht über die Vorlagengröße (" + tplW + "x" + tplH + ") hinaus.";
    if (warn) warn(msg);
  }
  ...
  return {
    canvas: out, posLeft, posTop, finalWidth, finalHeight,
    areaLeft: left, areaTop: top, areaWidth, areaHeight,
  };
}
