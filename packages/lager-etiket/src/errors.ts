 ...
 */
export { EtiketError, InvalidInputError, CapacityError, CheckDigitError } from "etiket/errors"
...
export const appErrors = {
  entriesEmpty: (): AppError => new AppError("ENTRIES_EMPTY", "Keine Lagerplätze eingegeben."),
  ...
  templateMissing: (): AppError =>
    new AppError("TEMPLATE_MISSING", "Bitte zuerst eine Vorlage laden (Galerie oder Datei)."),
...
export function isEtiketError(err: unknown): err is import("etiket/errors").EtiketError {
  return (
    ...
    err.name in { EtiketError: 1, InvalidInputError: 1, CapacityError: 1, CheckDigitError: 1 }
  )
}
...
export function describeEtiketError(err: import("etiket/errors").EtiketError): string {
  ...
  if (err.name === "CapacityError") {
    return "Der Code passt nicht in die gewählte Symbologie (zu lang). " + "Original: " + orig
  }
  if (err.name === "CheckDigitError") {
    return (
      "Die Prüfziffer stimmt nicht — letzte Ziffer weglassen, dann wird " +
      "sie automatisch berechnet. Original: " +
      orig
    )
  }
  // InvalidInputError (Basisklasse von CheckDigitError — daher vorher geprüft)
  return "Der Code enthält Zeichen, die Code 128 nicht darstellen kann. " + "Original: " + orig
}
