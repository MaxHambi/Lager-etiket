...
export function expandRange(start: string, end: string): string[] {
  ...
  if (!/^\d+$/.test(sNum) || !/^\d+$/.test(eNum)) {
    throw new Error(
      'Ungültiger Bereich "' + s + '" bis "' + e + '": Nach dem gemeinsamen ' +
      "Präfix \"" + prefix + "\" sind nur Ziffern erlaubt (gleich lang, führende Nullen möglich).",
    );
  }
  if (sNum.length !== eNum.length) {
    throw new Error(
      'Ungültiger Bereich "' + s + '" bis "' + e + '": Die Ziffern-Anteile müssen ' +
  ...
  if (count > MAX_RANGE_SIZE) {
    throw new Error(
      "Bereich zu groß (" + count + " Einträge). Maximal " + MAX_RANGE_SIZE + " pro Bereich erlaubt.",
    );
  }
