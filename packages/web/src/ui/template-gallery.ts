...
export class TemplateGallery {
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      this.log.warn(
        "Vorlagen-Galerie nicht verfügbar (" +
          (err as Error).message +
          ") — bitte Datei direkt laden.",
      )
      return
    }
  ...
  restoreLast(file: string, log: Logger): void {
    ...
    if (!card) return // Klick simulieren — derselbe Pfad wie manuelle Auswahl (Laden, Listener)
  ...
  private select(card: HTMLButtonElement, entry: TemplateManifestEntry): void {
    ...
    img.onload = (): void => {
      ...
      $("tplMeta").textContent =
        entry.label + " — " + img.naturalWidth + "×" + img.naturalHeight + " px"
