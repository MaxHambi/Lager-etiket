...
export class BatchesUI {
  ...
  /** Sichere Array-Zugriffe (noUncheckedIndexedAccess) — intern garantiert gefüllt. */
  private batchAt(i: number): BatchSection {
    const b = this.batches[i]
    if (!b) throw new Error(`Unterkategorie ${i + 1} nicht gefunden (interner Fehler).`)
    return b
  }
  ...
  constructor(private readonly log: Logger) {
    ...
    $("btnLoadEntries").addEventListener("click", () =>
      ($("entriesFile") as HTMLInputElement).click(),
    )
  ...
  private fillConfigSelect(b: BatchSection): void {
    ...
    b.config.innerHTML =
      ...
      this.configOptions
        .map(
          (c) => '<option value="' + escapeAttr(c.file) + '">' + escapeHtml(c.label) + "</option>",
        )
        .join("")
  ...
  private fillTemplateSelect(b: BatchSection): void {
    ...
    b.template.innerHTML =
      ...
      this.templateOptions
        .map(
          (t) => '<option value="' + escapeAttr(t.file) + '">' + escapeHtml(t.label) + "</option>",
        )
        .join("")
  ...
  addEntriesBatch(entries: string[]): boolean {
    ...
    const first = entries[0] as string
    const last = entries[entries.length - 1] as string
    section.start.value = first
    section.end.value = last
    this.log.ok(
      "Unterkategorie aus Eintragsliste erstellt: " +
        entries.length +
        " Einträge (" +
        first +
        " … " +
