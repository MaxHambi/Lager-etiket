...
export class ConfigLibrary {
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      this.log.warn(
        "Config-Bibliothek nicht verfügbar (" +
          (err as Error).message +
          ") — Formular/Import weiterhin nutzbar.",
      )
      return
    }
  ...
  async applyDefault(): Promise<void> {
    const entry =
      this.entries.find((c) => (c as ConfigManifestEntryWithDefault).default === true) ??
      this.entries.find((c) => c.file === DEFAULT_CONFIG_FILE)
    if (!entry) return
    try {
      const res = await fetch("public/configs/" + encodeURIComponent(entry.file), {
        cache: "no-cache",
      })
      if (!res.ok) throw new Error("HTTP " + res.status)
  ...
  async applySelected(): Promise<void> {
    ...
    try {
      ...
      this.log.err(
        'Konfiguration "' + file + '" konnte nicht geladen werden: ' + (err as Error).message,
      )
    }
  }
  ...
  dumpCurrent(): void {
    ...
    this.log.info(
      "Aktuelle Konfiguration als JSON ausgegeben — in public/configs/ ablegen + ins Manifest eintragen.",
    )
  }
